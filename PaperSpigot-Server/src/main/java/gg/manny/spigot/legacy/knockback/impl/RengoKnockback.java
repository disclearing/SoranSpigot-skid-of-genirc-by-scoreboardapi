...
@Getter
public class RengoKnockback extends Knockback {

	private double friction = 5.0D;
	private double horizontal = 0.33D;
	private double vertical = 0.5D;
	private double verticalLimit = 0.365D;
	private double extraHorizontal = 0.5D;
	private double extraVertical = 0D;

	@Override
	public KnockbackType getKnockback() {
		return KnockbackType.RENGO;
	}
	...
	public void loadConfig(GenericSpigotConfig config) {
		...
		this.friction = config.getDouble(prefix + "friction", 5.0D);
		
		this.horizontal = config.getDouble(prefix + "horizontal", 0.33D);
		this.vertical = config.getDouble(prefix + "vertical", 0.5D);
		
		this.extraHorizontal = config.getDouble(prefix + "extraHorizontal", 0.365D);
		this.extraVertical = config.getDouble(prefix + "extraVertical", 0.5D);
		this.verticalLimit = config.getDouble(prefix + "verticalLimit", 0D);
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args[1].equalsIgnoreCase("reset")) {
            ...
            config.set(getConfigPrefix() + ".extraVertical", newValue);
            ...
            config.set(getConfigPrefix() + ".verticalLimit", newValue);
