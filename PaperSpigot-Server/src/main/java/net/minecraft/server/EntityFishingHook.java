    ...
    public int e() {
        if (this.world.isStatic) {
            ...
            if (this.hooked != null) {
                ...
                double d4 = 0.1D;
                if (this.hooked instanceof EntityPlayer) {
                    double d0 = this.hooked.locX - this.locX;
                    double d2 = this.hooked.locZ - this.locZ;
                    while (d0 * d0 + d2 * d2 < 1.0E-4) {
                        d0 = (Math.random() - Math.random()) * 0.01;
                        d2 = (Math.random() - Math.random()) * 0.01;
                    }
//                    double magnitude = Entity.invSqrt(d0 * d0 + d2 * d2);
//                    this.hooked.motX /= GenericSpigotConfig.knockbackFriction;
//                    this.hooked.motY /= GenericSpigotConfig.knockbackFriction;
//                    this.hooked.motZ /= GenericSpigotConfig.knockbackFriction;
//                    this.hooked.motX = - d0 * magnitude * GenericSpigotConfig.knockbackHorizontal;
//                    this.hooked.motY += GenericSpigotConfig.knockbackVertical;
//                    this.hooked.motZ = - d2 * magnitude * GenericSpigotConfig.knockbackHorizontal;
//                    this.hooked = null;
                    Knockback knockback;
                    if (this.hooked.getBukkitEntity() instanceof CraftPlayer && ((CraftPlayer) this.hooked.getBukkitEntity()).getKnockback() != null) {
                        knockback = ((CraftPlayer) this.hooked.getBukkitEntity()).getKnockback();
                    } else {
                        knockback = GenericSpigotConfig.activeKnockback;
                    }
                    knockback.onHook(this, d0, d2);
                    ...
                    this.hooked.motY += d1 * d4 + (double)MathHelper.sqrt(d3) * 0.08;
