...
import gg.manny.spigot.util.CraftPotionUtil;
...
public abstract class EntityLiving extends Entity {
    ...
    public void addEffect(MobEffect mobeffect) {
        if (this.d(mobeffect)) {
            ...
            MobEffect mobeffect1 = (MobEffect) this.effects.get(mobeffect.getEffectId());
            if (mobeffect1 == null) {
                ...
                PotionEffectExtendEvent event = new PotionEffectExtendEvent((LivingEntity) this.getBukkitEntity(),
                        CraftPotionUtil.toBukkit(mobeffect),
                        CraftPotionUtil.toBukkit((MobEffect) this.effects.get(mobeffect.getEffectId())));
                this.world.getServer().getPluginManager().callEvent(event);
                if (event.isCancelled()) return;
                // SportBukkit end
        ...
        if (this.isInvulnerable()) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (knockbackCancelled)
                    this.world.broadcastEntityEffect(this, (byte) 2); // PaperSpigot - Disable explosion knockback
    ...
    public void die(DamageSource damagesource) {
        ...
        if (!this.world.isStatic) {
            ...
            if (this.aG() && this.world.getGameRules().getBoolean("doMobLoot")) {
                ...
                if (GenericSpigot.INSTANCE.getConfig().isMobStackingEnabled()) {
                    EntityInsentient entityinsentient = entity instanceof EntityInsentient ? (EntityInsentient) entity : null;
                    if (entityinsentient != null && entityinsentient.getCustomName().equals(CC.GREEN + "x" + GenericSpigot.INSTANCE.getConfig().getMobStackingMultiplier())) {
                        int multiplier = GenericSpigot.INSTANCE.getConfig().getMobStackingMultiplier();
    ...
    public void a(Entity entity, float f, double d0, double d1) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            this.al = true;
            Knockback knockback;
            if (this.bukkitEntity instanceof CraftPlayer && ((CraftPlayer) this.bukkitEntity).getKnockback() != null) {
                knockback = ((CraftPlayer) this.bukkitEntity).getKnockback();
            } else {
                knockback = GenericSpigotConfig.activeKnockback;
            }
            knockback.a(this, entity, d0, d1);
        }
    }
    ...
    protected void bo() {
        if (GenericSpigot.INSTANCE.getConfig().isDisableEntityCollisions()) {
            return;
        }
