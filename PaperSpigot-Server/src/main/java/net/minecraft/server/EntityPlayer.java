...
public class EntityPlayer extends EntityHuman implements ICrafting {
    ...
    public void h() {
        ...
        if (!this.chunkCoordIntPairQueue.isEmpty()) {
            ...
            ArrayList<TileEntity> arraylist1 = new ArrayList<>();
            while (iterator1.hasNext() && arraylist.size() < this.world.spigotConfig.maxBulkChunk) {
                ...
                if (chunkcoordintpair != null) {
                    Chunk chunk;
                    if (!this.world.isLoaded(chunkcoordintpair.x << 4, 0, chunkcoordintpair.z << 4) || !(chunk = this.world.getChunkAt(chunkcoordintpair.x, chunkcoordintpair.z)).isReady())
                        continue;
                    arraylist.add(chunk);
    ...
    public void i() {
        try {
            ...
            if (!GenericSpigot.INSTANCE.getConfig().isDisableTickingMaps() && this.tickCount++ % 100 == 0) {
    ...
    public void sendDestroyPacket(Entity entity) {
