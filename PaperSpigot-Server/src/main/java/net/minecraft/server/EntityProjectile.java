...
public abstract class EntityProjectile extends Entity implements IProjectile {
    ...
    protected double lastSuitableX = -1.0D;
    protected double lastSuitableY = -1.0D;
    protected double lastSuitableZ = -1.0D;
    ...
    public void shoot(double d0, double d1, double d2, float f, float f1) {
        ...
        this.i = 0;
        this.lastSuitableX = this.locX;
        this.lastSuitableY = this.locY;
        this.lastSuitableZ = this.locZ;
    }

    private boolean isSuitableLocation(double x, double y, double z) {
        return !this.world.c(new AxisAlignedBB(x, y, z, x, y + 1.0D, z));
    }
    ...
    public void h() {
        ...
        if (GenericSpigot.INSTANCE.getConfig().isEnderpearlTaliban()) {
            ...
            //Handles passable blocks
            if (movingobjectposition != null && movingobjectposition.type == EnumMovingObjectType.BLOCK && this instanceof EntityEnderPearl) {
                int x = movingobjectposition.b;
                int y = movingobjectposition.c;
                int z = movingobjectposition.d;

                Block block = this.world.getType(x, y, z);
                if (GenericSpigot.INSTANCE.getConfig().isEnderpearlGates() && block == Blocks.FENCE_GATE) {
                    if ((this.world.getData(x, y, z) & 0x4) != 0) {
                        movingobjectposition = null;
                    }
                } else if (GenericSpigot.INSTANCE.getConfig().isEnderpearlTripwire() && block == Blocks.TRIPWIRE) {
                    movingobjectposition = null;
                }
            }
            ...
            //nigga shit for taliban pearl fixing
            //fuck viper bahah
            if (this.isSuitableLocation(this.locX, this.locY, this.locZ)) {
                this.lastSuitableX = this.locX;
                this.lastSuitableY = this.locY;
                this.lastSuitableZ = this.locZ;
            }
            else if (this.isSuitableLocation(this.locX, Math.floor(this.locY), this.locZ)) {
                this.lastSuitableX = this.locX;
                this.lastSuitableY = Math.floor(this.locY);
                this.lastSuitableZ = this.locZ;
            } else {
                Block type = this.world.getType(this.blockX, this.blockY, this.blockZ);
                if (type == Blocks.AIR && this.world.getType(this.blockX, this.blockY - 1, this.blockZ) == Blocks.AIR && this.isSuitableLocation(this.locX, this.blockY - 1, this.blockZ)) {
                    this.lastSuitableX = this.locX;
                    this.lastSuitableY = (this.blockY - 1);
                    this.lastSuitableZ = this.locZ;
                }
            }
        } else {
    ...
    public void z() {
        ...
        if (movingobjectposition != null && movingobjectposition.type == EnumMovingObjectType.BLOCK && this instanceof EntityEnderPearl) {
            ...
            Block block = this.world.getType(x, y, z);
            if (GenericSpigot.INSTANCE.getConfig().isEnderpearlGates() && block == Blocks.FENCE_GATE) {
                if ((this.world.getData(x, y, z) & 4) != 0) {
                	movingobjectposition = null;
                }
            } else if (GenericSpigot.INSTANCE.getConfig().isEnderpearlTripwire() && block == Blocks.TRIPWIRE) {
                movingobjectposition = null;
