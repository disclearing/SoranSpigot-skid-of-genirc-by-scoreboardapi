...
import gg.manny.spigot.events.PacketCustomPayloadEvent;
...
public class NetworkManager extends SimpleChannelInboundHandler {
    ...
    public void a(EnumProtocol enumprotocol) {
        this.lastProtocol = (EnumProtocol) this.m.attr(d).getAndSet(enumprotocol);
    ...
    protected void a(ChannelHandlerContext channelhandlercontext, Packet packet) {
        if (this.m.isOpen()) {
            ...
            if (this.packetListener instanceof PlayerConnection) {
                PlayerConnection connection = (PlayerConnection) this.packetListener;
                if (packet instanceof PacketPlayInCustomPayload) {
                    PacketPlayInCustomPayload payload = (PacketPlayInCustomPayload) packet;
                    PacketCustomPayloadEvent event = new PacketCustomPayloadEvent(connection.getPlayer(), payload.c(), payload);
                    connection.getPlayer().getServer().getPluginManager().callEvent(event);
                }
            }
        }
        if (this.packetListener instanceof PlayerConnection) {
            try {
                for (PacketHandler handler : GenericSpigot.INSTANCE.getPacketHandlers()) {
                    handler.handleReceivedPacket((PlayerConnection) this.packetListener, packet);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void a() {
        ...
        if (this.lastProtocol != enumprotocol) {
            if (this.lastProtocol != null) {
                this.packetListener.a(this.lastProtocol, enumprotocol);
            }

            this.lastProtocol = enumprotocol;
        }

        if (this.packetListener != null) {
            // PaperSpigot start - Improve Network Manager packet handling - Configurable packets per player per tick processing
            Packet packet;
            for (int i = PaperSpigotConfig.maxPacketsPerPlayer; (packet = (Packet) this.k.poll()) != null && i >= 0; --i) {
                // PaperSpigot end
