...
import gg.manny.spigot.events.PlayerAttackEvent;
import gg.manny.spigot.events.ReachEvent;
...
public class PlayerConnection implements PacketPlayInListener {
    ...
    private int packetsNotReceived = 0;
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (!this.player.viewingCredits) {
            ...
            if (packetplayinflying.hasPos && delta > 0.0D && this.checkMovement && !this.player.dead) {
                for (SimpleMovementHandler handler : GenericSpigot.INSTANCE.getSimpleMovementHandlers()) {
                    try {
                        handler.onPlayerMove(player, to, from);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }

                for (MovementHandler handler : GenericSpigot.INSTANCE.getMovementHandlers()) {
                    try {
            ...
            if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
                ...
                if (!GenericSpigot.INSTANCE.getConfig().isDisableEventPlayerMove()) {
        ...
        if (packetplayinblockdig.g() == 4) {
            ...
            if (packetplayinblockdig.g() == 0) {
                if (!this.minecraftServer.a(worldserver, i, j, k, this.player)) {
                    ...
                    if (!GenericSpigot.INSTANCE.getConfig().isDisableEventLeftClickBlock()) {
    ...
    public void a(PacketPlayInBlockPlace packetplayinblockplace) {
        ...
        if (packetplayinblockplace.getFace() == 255) {
            ...
            if ((reachDistance + this.getPlayer().getReachDistance()) > (this.getPlayer().getGameMode() == org.bukkit.GameMode.CREATIVE ? CREATIVE_PLACE_DISTANCE_SQUARED : SURVIVAL_PLACE_DISTANCE_SQUARED)) {
                return;
            }
    ...
    public void sendPacket(Packet packet) {
        ...
        try {
            ...
            for (PacketHandler handler : GenericSpigot.INSTANCE.getPacketHandlers()) {
                try {
    ...
    private void handleCommand(String s) {
        co.aikar.timings.SpigotTimings.playerCommandTimer.startTiming(); // Spigot
        ...
        if (event.isCancelled()) {
            co.aikar.timings.SpigotTimings.playerCommandTimer.stopTiming(); // Spigot
            return;
        }
    ...
    public void a(PacketPlayInArmAnimation packetplayinarmanimation) {
        if (this.player.dead) return; // CraftBukkit
        if (GenericSpigot.INSTANCE.getConfig().isInvalidArmAnimationKick()) {
        ...
        if (packetplayinarmanimation.d() == 1) {
            ...
            if (!GenericSpigot.INSTANCE.getConfig().isDisableEventLeftClickAir()) {
                float f = 1.0F;
    ...
    private int lastUseTick = MinecraftServer.currentTick;
    ...
    public void a(PacketPlayInUseEntity packetplayinuseentity) {
        ...
        if (this.lastUseTick != MinecraftServer.currentTick) {
            this.lastUseTick = MinecraftServer.currentTick;
            this.lastUseCount = 0;
        } else if (10 <= this.lastUseCount++) {
            Bukkit.getLogger().info(String.valueOf(this.player.getName()) + " was ratelimited for PPIUE.");
            return;
        }
        WorldServer worldserver = this.minecraftServer.getWorldServer(this.player.dimension);
        ...
        if (entity != null) {
            ...
            } else if (GenericSpigot.INSTANCE.getConfig().isAnticheatEnabled()) {
                Bukkit.getPluginManager().callEvent(new ReachEvent(this.player, this.player.f(entity)));
                this.getPlayer().setReachDistance(this.player.f(entity));
            }
        }
    }
