...
public abstract class World implements IBlockAccess {
    ...
    protected void a(Entity entity) {
        ...
        entity.worldOwner = this.worldLock;
    }
    ...
    public void tickEntities() {
        ...
        for (i = 0; i < this.i.size(); ++i) {
            ...
            if (entity.dead || entity.worldOwner != this.worldLock) {
                this.i.remove(i--);
            }
        }
        ...
        if (!GenericSpigot.INSTANCE.getConfig().isDisableTileEntityTick()) {
    ...
    public List<EntityPlayer> findNearbyPlayersClassic(double targetX, double targetY, double targetZ, double radius) {
        ArrayList list = Lists.newArrayList();
        double radiusSquared = radius * radius;
        int i = 0;
        while (i < this.players.size()) {
            EntityPlayer player = (EntityPlayer)this.players.get(i);
            if (!(player == null || player.dead || radius >= 0.0 && player.distanceSquared(targetX, targetY, targetZ) > radiusSquared)) {
                list.add(player);
            }
            ++i;
        }
    ...
    public List<EntityPlayer> findNearbyPlayers(double targetX, double targetY, double targetZ, double radius) {
        if (64.0 <= radius || radius < 0.0) {
            return this.findNearbyPlayersClassic(targetX, targetY, targetZ, radius);
        }
        ArrayList list = Lists.newArrayList();
        int targetChunkX = MathHelper.floor(targetX / 16.0);
        int targetChunkZ = MathHelper.floor(targetZ / 16.0);
        int chunkRadius = (int)Math.ceil(radius / 16.0);
        int x = targetChunkX - chunkRadius;
        while (x <= targetChunkX + chunkRadius) {
            int z = targetChunkZ - chunkRadius;
            while (z <= targetChunkZ + chunkRadius) {
                Chunk chunk = this.getChunkIfLoaded(x, z);
                if (chunk != null) {
                    for (EntityPlayer currentPlayer : chunk.getPlayers()) {
                        if (currentPlayer == null || currentPlayer.dead) continue;
                        double distanceSquared = currentPlayer.distanceSquared(targetX, targetY, targetZ);
