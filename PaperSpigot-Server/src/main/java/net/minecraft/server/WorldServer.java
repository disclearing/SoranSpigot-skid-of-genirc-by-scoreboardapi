...
public class WorldServer extends World {
    ...
    private List<NextTickListEntry> V = new ArrayList() {
        public boolean add(NextTickListEntry o) {
            if (GenericSpigot.INSTANCE.getConfig().isDisableTileEntityTick()) return false;
            return super.add(o);
        }
    };
    ...
    public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, String s, int i, WorldSettings worldsettings, MethodProfiler methodprofiler, org.bukkit.World.Environment env, org.bukkit.generator.ChunkGenerator gen) {
        ...
        if (GenericSpigot.INSTANCE.getConfig().isDisableTileEntityTick()) {
            this.N = new HashTreeSet() {
                public boolean add(NextTickListEntry object) {
                    return true;
                }
    ...
    public void doTick() {
        if (!GenericSpigot.INSTANCE.getConfig().isDisableTickingWeather()) {
            super.doTick();
        }
        ...
        if (!GenericSpigot.INSTANCE.getConfig().isDisableBiomeCacheCleanup()) {
            this.worldProvider.e.b();
        }

        if (!GenericSpigot.INSTANCE.getConfig().isDisableTickingSleepCheck() && everyoneDeeplySleeping()) {
        ...
        if(!GenericSpigot.INSTANCE.getConfig().isDisableBlockTick()) {
            this.g();
        }
        ...
        if(!GenericSpigot.INSTANCE.getConfig().isDisableTickingVillages()) {
    ...
    private void Y() {
        // CraftBukkit start
        if(GenericSpigot.INSTANCE.getConfig().isDisableWeather()) {
    ...
    protected void a(WorldSettings worldsettings) {
        ...
        if (this.N == null) {
            if (GenericSpigot.INSTANCE.getConfig().isDisableTileEntityTick()) {
                ...
                this.N = new HashTreeSet<>(); // PaperSpigot
            }
        }
