...
public class CraftPlayer extends CraftHumanEntity implements Player {
    ...
    private double reachDistance = -1;
    ...
    @Deprecated
    public void setKnockbackByType(KnockbackType knockbackByType) {
        this.knockback = GenericSpigot.INSTANCE.getConfig().getKnockbackByType(knockbackByType);
    }
    ...
    public boolean canSeeExtra(org.bukkit.entity.Entity entity) {
        if (this == entity) {
            return true;
        }
        ...
        if (nmsEntity instanceof EntityProjectile) {
            ...
            if (entityArrow.shooter instanceof EntityPlayer && !((EntityPlayer)entityArrow.shooter).getBukkitEntity().canSee(this)) {
                return this.canSee(((EntityPlayer)entityArrow.shooter).getBukkitEntity());
            }
        } else if (nmsEntity instanceof EntityItem) {
            final EntityItem entityItem = (EntityItem)nmsEntity;
            if (entityItem.getThrower() instanceof EntityPlayer) {
                return this.canSee(((EntityPlayer)entityItem.getThrower()).getBukkitEntity());
            }
    ...
    public void hidePlayer(Player player) {
        ...
        EntityTrackerEntry entry = tracker.getTrackedEntity(other.getId());
        if (entry != null) {
        ...
        if (GenericSpigot.INSTANCE.getConfig().isHidePlayersFromTab()) {
            getHandle().playerConnection.sendPacket(PacketPlayOutPlayerInfo.removePlayer( ( (CraftPlayer) player ).getHandle ()));
        }
    }
